...
(function() {
	...
	function mission_skin_post_header_init() {
	...
	mission_add_filter( 'trx_addons_filter_skills_pie_options', function(opt) {
	...
	mission_add_filter( 'trx_addons_filter_slider_init_args', function( $param, $init ) {
	...
	function skin_blogger_toggle_filter_add_element() {
		if ( jQuery('.content').find(".sc_style_toggle").length > 0 ) {
			jQuery(" .sc_style_toggle").each(function() {
				var toggle_title = MISSION_STORAGE['toggle_title'];
						...
						$share_items
							...
							.on('click', '.social_item[data-copy-link-url]', function(e) {
								...
								if ( url != '' ) {
									...
									var msg = $self.data('message') ? $self.data('message') : MISSION_STORAGE['msg_copied'];
									if ( msg ) {
	...
	// Audio
	function mission_skin_audio_init() {
	...
	jQuery( window ).load(function() {
		...
		mission_skin_post_header_init();

	});

	jQuery( document ).on(
		'action.ready_mission', function() {
			...
			jQuery('.tribe-events-view').on( 'beforeAjaxComplete.tribeEvents beforeAjaxSuccess.tribeEvents beforeAjaxError.tribeEvents', mission_tribe_events_after_ajax );
			function mission_tribe_events_after_ajax( jqXHR, textStatus ) {
				setTimeout( function() {
					...
					btn.on( 'click', function(e) {
						...
						if ( $i_agree.length > 0 ) {
							if ( ! $i_agree.get(0).checked ) {
								form.append( '<div class="trx_addons_message_box trx_addons_message_box_error">' + MISSION_STORAGE['msg_i_agree_error'] + '</div>' );
						...
						var rez = mission_form_validate(form, {
							...
							rules: [
								{
									field: 'EMAIL',
									min_length: {value: 7, message: MISSION_STORAGE['mc4wp_msg_email_min']},
									max_length: {value: 60, message: MISSION_STORAGE['mc4wp_msg_email_max']},
									mask: {
			...
			mission_add_filter( 'mission_filter_mobile_menu_close_timeout', function( timeout, $menu ) {
				return 1000;
			} );
            ...
            function mission_skin_open_panel( id ) {
            ...
            jQuery( '.trx_addons_panel_menu_link:not(.panel_inited)' )
				...
				.on( 'click', function(e) {
					mission_skin_open_panel( jQuery(this).attr('href').substring(1) );
					e.preventDefault();
					return false;
				});

            // Close panel
            function mission_skin_close_panel() {
        ...
        jQuery( $search_wrap.each( function() {
            ...
            search_field.on('keyup', function(e) {
                if (e.keyCode === 27) {
                    ...
                    setTimeout(function () {
                        mission_search_close($search_wrap_init);
    ...
    function mission_search_close($search_wrap_init) {
	...
	jQuery( document ).on(
		'action.scroll_mission', function() {
	...
	jQuery( document ).on(
		'action.resize_mission', function() {
			extra_margin_top_video_popup_button();
			mission_skin_post_header_init();
			skin_add_custom_width_filter_list();
		}
